...
def Wfast(img,nstains,lamb,num_patches,patchsize,level,background_correction,pool):
...
#     out = suppress_stdout()
    Ws, model = spams.trainDL(np.asfortranarray(np.transpose(VforW)),return_model=True,**param)
    print(model)
    print(Ws)
#     suppress_stdout(out)
    return Ws
